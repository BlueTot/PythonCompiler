...
def shift_pointers(assembly_code, start_line, split_point=None, only_pointers=False):
    output = {}
...
def compile_argument(expression, used_reg=None): # Compile argument
    return compile_rpn(convert_expression(expression).split(","), used_reg=used_reg)
    ...
    try:
        while ln < len(code):
            ...
            if re.match(r"if.*:", line) or re.match(r"elif.*:", line) or re.match(r"while.*:", line): # If statement, elif statement or while loop
                ...
                assembly_code = extend_code(assembly_code, __compile_code(code[ln+1:end_of_curr_if_block]), linenum) # Compile whatever code there is inside the if block
                ...
                assembly_code = extend_code(assembly_code, __compile_code(loop_code), linenum)
                ln = end_of_for_loop
                ...
                linenum = len(assembly_code)
                end_of_if_statement = find_end_of_if_statement(code, ln+1, num_indents(code, ln)) # Find when the if statement ends
                assembly_code = extend_code(assembly_code, __compile_code(code[ln+1:end_of_if_statement]), linenum) # Compile whatever code there is inside the if block
                ...
                if not is_number(size):
                    new_code, size = compile_argument(size)
                ...
                if not is_value(index):
                    new_code1, index = compile_argument(index)
                    assembly_code = extend_code(assembly_code, new_code1, linenum)
                if not is_value(expression):
                    new_code2, expression = compile_argument(expression, used_reg=index if not index.isdigit() else None)
                    assembly_code = extend_code(assembly_code, new_code2, len(assembly_code))
                ...
                assembly_code = extend_code(assembly_code, __compile_code([f"{variable} = {variable} + ({operand})"]), linenum) # Convert fast addition operator into compilable syntax
                ln += 1
            
            elif re.match(r".*-=.*", line): # Fast Subtraction operator
                variable, operand = line.split("-=")
                assembly_code = extend_code(assembly_code, __compile_code([f"{variable} = {variable} - ({operand})"]), linenum) # Convert fast subtraction operator into compilable syntax
                ln += 1
            
            elif re.match(r".*\*=.*", line): # Fast Multiplication operator
                variable, operand = line.split("*=")
                assembly_code = extend_code(assembly_code, __compile_code([f"{variable} = {variable} * ({operand})"]), linenum) # Convert fast multiplication operator into compilable syntax
                ln += 1
            
            elif re.match(r".*/=.*", line): # Fast Division operator
                variable, operand = line.split("/=")
                assembly_code = extend_code(assembly_code, __compile_code([f"{variable} = {variable} / ({operand})"]), linenum) # Convert fast division operator into compilable syntax
                ln += 1
            
            elif re.match(r".*^=.*", line): # Fast Exponentiation operator
                variable, operand = line.split("^=")
                assembly_code = extend_code(assembly_code, __compile_code([f"{variable} = {variable} ^ ({operand})"]), linenum) # Convert fast exponentiation operator into compilable syntax
                ln += 1
            
            elif re.match(r".*%=.*", line): # Fast Modulo operator
                variable, operand = line.split("%=")
                assembly_code = extend_code(assembly_code, __compile_code([f"{variable} = {variable} % ({operand})"]), linenum) # Convert fast modulo operator into compilable syntax
                ln += 1
            
            elif re.match(r".*\\=.*", line): # Fast Floor Division operator
                variable, operand = line.split("\\=")
                assembly_code = extend_code(assembly_code, __compile_code([f"{variable} = {variable} \ ({operand})"]), linenum) # Convert fast floor division operator into compilable syntax
                ln += 1
                ...
                assembly_code = extend_code(assembly_code, __compile_code([f"{variable} = {variable} + 1"]), linenum) # Convert increment operator into compilable syntax
                ln += 1
            
            elif re.match(r".*--", line): # Decrement operator
                variable = line.replace("--", "")
                assembly_code = extend_code(assembly_code, __compile_code([f"{variable} = {variable} - 1"]), linenum) # Convert decrement operator into compilable syntax
                ln += 1

            elif re.match(r"print(.*)", line): # Print Statement
                argument = line.split("print(")[1][:-1]
                if not is_number(argument):
                    new_code, argument = compile_argument(argument)
                    assembly_code = extend_code(assembly_code, new_code, linenum)
...
def remove_pass_statements(assembly_code): # Remove pass statements from compiled code
    ln = 0
    while ln < len(assembly_code) - 1:
        line = assembly_code[ln]
        if line == "PASS":
            next_line = __next_non_pass_line(assembly_code, ln)
            for loc in __find_pointers_to(assembly_code, ln):
                assembly_code[loc].replace(str(ln), str(next_line))
            assembly_code = extend_code(shift_pointers(__extract_section(assembly_code, 0, ln), -1, split_point=ln, only_pointers=True), 
                                        shift_pointers(__extract_section(assembly_code, ln+1, len(assembly_code)), -1, split_point=ln), 0)
            ln = 0
        ln += 1
...
def compile_code(code): # Compile code function without pass statements
    return remove_pass_statements(__compile_code(code))
