    ...
    try:
        while ln < len(code):
            ...
            if re.match(r"if.*:", line) or re.match(r"elif.*:", line): # If statement or elif statement
                ...
                linenum = len(assembly_code)
                for k, v in shift_pointers(compile_comparison(line), linenum).items():
                    assembly_code[k] = v
                ...
                for k, v in shift_pointers(compile_assignment(line), linenum).items(): # Compile assignment RPN into multiple statements
                    assembly_code[k] = v
                ln += 1
